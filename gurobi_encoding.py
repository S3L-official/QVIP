...
class QNNEncoding_gurobi:
    ...
    def sat(self, mode, path, sample_id, timeout=None, verbose=False):
        ...
        if mode == "gp":
            ...
            smt_solving_start_time = time.time()
            self._stats["build_time"] = smt_solving_start_time - build_start_time
            print("\n The encoding time is: " + str(self._stats["build_time"]))
            print(
                "\n==================================== Now we start do smt-based solving! ====================================")

            result_smt = self.btor.Sat()
            smt_end_time = time.time()
            self._stats["smt_sat_time"] = smt_end_time - smt_solving_start_time

            print("\n===== ===== SMT sat time is: " + str(self._stats["smt_sat_time"]) + " ===== =====\n")
...
def check_robustness_gurobi(encoding, x, y, eps, mode, path, sample_id):  # encoding is a SMT model to solve
    """
        Return values:
    ...
    attack = None
    ifSat = encoding.sat(mode, path, sample_id, verbose=False)
    if ifSat:
        if mode == "gp":
